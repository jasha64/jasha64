...
public:
  ...
  int Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], std::function<bool(DATATYPE, std::vector<DATATYPE>&)> a_resultCallback, std::vector<DATATYPE>& a_context);
  
  int SearchTri(const Point& a_p1, const Point& a_p2, const Point& a_p3, std::function<bool(DATATYPE, std::vector<DATATYPE>&)> a_resultCallback, std::vector<DATATYPE>& a_context);
...
protected:
  ...
  bool RectTriOverlap(const Point& a_p1, const Point& a_p2, const Point& a_p3, Rect* a_rect);
  bool PointInRect(const Point& a_p, Rect* a_rect);
  bool PointInTri(const Point& a_p1, const Point& a_p2, const Point& a_p3, const Point& a_q);
  bool PointInTri(const Point& a_p1, const Point& a_p2, const Point& a_p3, Rect* a_rect);
  void ReInsert(Node* a_node, ListNode** a_listNode);
  bool Search(Node* a_node, Rect* a_rect, int& a_foundCount, std::function<bool(DATATYPE, std::vector<DATATYPE>&)> a_resultCallback, std::vector<DATATYPE>& a_context);
  bool SearchTri(Node* a_node, const Point& a_p1, const Point& a_p2, const Point& a_p3, int& a_foundCount, std::function<bool(DATATYPE, std::vector<DATATYPE>&)> a_resultCallback, std::vector<DATATYPE>& a_context);
...
RTREE_TEMPLATE
int RTREE_QUAL::Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], std::function<bool(DATATYPE, std::vector<DATATYPE>&)> a_resultCallback, std::vector<DATATYPE>& a_context)
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
...
RTREE_TEMPLATE
int RTREE_QUAL::SearchTri(const Point& a_p1, const Point& a_p2, const Point& a_p3, std::function<bool(DATATYPE, std::vector<DATATYPE>&)> a_resultCallback, std::vector<DATATYPE>& a_context)
{
#ifdef _DEBUG
	for (int index = 0; index<NUMDIMS; ++index)
	{
	...
	int foundCount = 0;
	SearchTri(m_root, a_p1, a_p2, a_p3, foundCount, a_resultCallback, a_context);
...
RTREE_TEMPLATE
bool RTREE_QUAL::PointInTri(const Point& a_p1, const Point& a_p2, const Point& a_p3, const Point& a_q)
{
	Point QA = a_p1 - a_q;
	Point QB = a_p2 - a_q;
	Point QC = a_p3 - a_q;
	double t1 = Cross(QA, QB);
	double t2 = Cross(QB, QC);
	double t3 = Cross(QC, QA);
	return t1 * t2 >= 0 && t1 * t3 >= 0;
}

RTREE_TEMPLATE
bool RTREE_QUAL::PointInRect(const Point& a_p, Rect* a_rect)
{
	double a_px = a_p.first, a_py = a_p.second;
	double a_x1 = a_rect->m_min[0], a_x2 = a_rect->m_max[0], a_y1 = a_rect->m_min[1], a_y2 = a_rect->m_max[1];
	return a_x1 < a_px && a_px < a_x2 && a_y1 < a_py && a_py < a_y2;
}

RTREE_TEMPLATE
bool RTREE_QUAL::RectTriOverlap(const Point& a_p1, const Point& a_p2, const Point& a_p3, Rect* a_rect)
{
	Point R1 = std::make_pair(a_rect->m_min[0], a_rect->m_min[1]);
	Point R2 = std::make_pair(a_rect->m_min[0], a_rect->m_max[1]);
	Point R3 = std::make_pair(a_rect->m_max[0], a_rect->m_min[1]);
	Point R4 = std::make_pair(a_rect->m_max[0], a_rect->m_max[1]);
	if (PointInTri(a_p1, a_p2, a_p3, R1) || PointInTri(a_p1, a_p2, a_p3, R2) ||
		PointInTri(a_p1, a_p2, a_p3, R3) || PointInTri(a_p1, a_p2, a_p3, R4)) return true;
	if (PointInRect(a_p1, a_rect) || PointInRect(a_p2, a_rect) || PointInRect(a_p3, a_rect)) return true;
	Point T[3] = { a_p2 - a_p1, a_p3 - a_p2, a_p1 - a_p3 };
	Point R[4] = { R2 - R1, R3 - R2, R4 - R3, R1 - R4 };
	if (SegmentIntersect(a_p1, a_p2, R1, R2) || SegmentIntersect(a_p1, a_p2, R2, R3) ||
		SegmentIntersect(a_p1, a_p2, R3, R4) || SegmentIntersect(a_p1, a_p2, R4, R1) ||
		SegmentIntersect(a_p2, a_p3, R1, R2) || SegmentIntersect(a_p2, a_p3, R2, R3) ||
		SegmentIntersect(a_p2, a_p3, R3, R4) || SegmentIntersect(a_p2, a_p3, R4, R1) ||
		SegmentIntersect(a_p3, a_p1, R1, R2) || SegmentIntersect(a_p3, a_p1, R2, R3) ||
		SegmentIntersect(a_p3, a_p1, R3, R4) || SegmentIntersect(a_p3, a_p1, R4, R1)) return true;
	return false;
}

//限用于case 2,3,5,6时搜索到叶子节点，此时a_rect一定是一个二维点
RTREE_TEMPLATE
bool RTREE_QUAL::PointInTri(const Point& a_p1, const Point& a_p2, const Point& a_p3, Rect* a_rect)
{
	Point P = std::make_pair(a_rect->m_min[0], a_rect->m_min[1]);
	return PointInTri(a_p1, a_p2, a_p3, P);
}
...
RTREE_TEMPLATE
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, int& a_foundCount, std::function<bool(DATATYPE, std::vector<DATATYPE>&)> a_resultCallback, std::vector<DATATYPE>& a_context)
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
...
RTREE_TEMPLATE
bool RTREE_QUAL::SearchTri(Node* a_node, const Point& a_p1, const Point& a_p2, const Point& a_p3, int& a_foundCount, std::function<bool(DATATYPE, std::vector<DATATYPE>&)> a_resultCallback, std::vector<DATATYPE>& a_context)
{
	ASSERT(a_node);
	ASSERT(a_node->m_level >= 0);
	//ASSERT(a_rect);
	...
	if (a_node->IsInternalNode()) // This is an internal node in the tree
	{
		for (int index = 0; index < a_node->m_count; ++index)
		{
			if (RectTriOverlap(a_p1, a_p2, a_p3, &a_node->m_branch[index].m_rect))
			{
				if (!SearchTri(a_node->m_branch[index].m_child, a_p1, a_p2, a_p3, a_foundCount, a_resultCallback, a_context))
				{
					return false; // Don't continue searching
				}
			}
	...
	else // This is a leaf node
	{
		for (int index = 0; index < a_node->m_count; ++index)
		{
			if (PointInTri(a_p1, a_p2, a_p3, &a_node->m_branch[index].m_rect))
			{
				DATATYPE& id = a_node->m_branch[index].m_data;
